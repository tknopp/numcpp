...
int main()
{
  ...
  auto x2 = x( slice(0,2), slice(0,2) );         // select block within x
  print(x2);                                     // x2 = [1 2; 4 5]

  auto x3 = x( slice(0,end,2), slice(0,end,2) ); // select strided block within x
  print(x3);                                     // x3 = [1 3; 7 8]

  auto x4 = x( full, 0 );                        // select first row of x
  print(x4);                                     // x4 = [1 2 3]
  ...
  std::vector<int> indices = {0, 1, 7, 5};       // create index vector
  auto u1 = u(indices);                          // non-regular indexing of u
  print(u1);                                     // u1 = [0 1 6 4]
}
